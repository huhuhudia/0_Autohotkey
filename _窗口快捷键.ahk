...
If DoubleAlt
{
    ...
    ; PostMessage,0x112,0xf020,,,ahk_id %KDE_id% ; 最小化 暂时停用
    WinGetPos, , , KDE_WinW, KDE_WinH, ahk_id %KDE_id%
    KDE_CX := (A_ScreenWidth - KDE_WinW) / 2
    KDE_CY := (A_ScreenHeight - KDE_WinH) / 2
    WinMove, ahk_id %KDE_id%, , KDE_CX, KDE_CY
    DoubleAlt := false
    return
}
; 获取初始的鼠标位置和窗口 id, 并
...
If DoubleAlt
{
    WinMove, ahk_id %KDE_id%, , A_ScreenWidth - KDE_WinW
    DoubleAlt := false
    return
}
WinMove, ahk_id %KDE_id%, , , 0
Return
...
WinMove, ahk_id %KDE_id%, , , A_ScreenHeight - KDE_WinH
Return
...
Sleep 0
KeyWait LWin  ; 这样避免了键盘的自动重复功能的扰乱.
return